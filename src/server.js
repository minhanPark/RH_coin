...
app.get("/transaction/:id", (req, res) => {
  ...
  if(tx === undefined){
    res.status(400).send("Transaction is not founded");
  }
  res.send(tx);
})

app.route("/transactions").get((req, res) =>{
  res.send(getMempool());
}).post((req, res) => {
  try {
...
const server = app.listen(PORT, () => console.log(`RH COIN Server Running on ${PORT}! Welcome`));
